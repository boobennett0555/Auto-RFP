  ...
  return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #0f2744 0%, #1a3a5c 60%, #0f2744 100%)' }}>
      ...
      <div className="relative z-10 w-full max-w-sm mx-4">
            ...
            <>
              ...
              <form onSubmit={handleLogin} className="space-y-4">
                ...
                <button
                  ...
                  className="w-full text-white py-3 rounded-xl text-sm font-semibold disabled:opacity-40 flex items-center justify-center gap-2 transition-opacity"
                  style={{ background: '#0f2744' }}
                >
                  {loading ? <><span className="spin">⟳</span> Sending…</> : 'Send Magic Link →'}
                </button>
              </form>
            </>
          ) : (
        ...
        <p className="text-center text-slate-600 text-xs mt-6 opacity-60">
          © {new Date().getFullYear()} RFP Studio · Internal team tool
        </p>
      </div>
    </div>
  )
